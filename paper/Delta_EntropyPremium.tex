...
\author{Junwen Zhang\thanks{Corresponding author. Email: junwen.zhang@xjtlu.edu.cn}}
\date{June 2026}
...
\textit{On the high correlation.} The $r = -0.685$ correlation between H\_smooth and inner confidence is expected: when agents are more confident individually, they also tend to agree more across agents. However, the two measures capture different economic mechanisms. Inner confidence reflects the \textit{precision} of each agent's signal (a within-agent property), while H\_smooth reflects the \textit{concentration} of the multi-agent belief distribution (an across-agent property). The fact that only the across-agent measure predicts returns suggests that the market prices the \textit{structure of disagreement} across information channels, not the precision of any single channel. We view H\_smooth and inner confidence as complementary: the former captures cross-sectional variation in belief structure, while the latter captures within-agent certainty.
